...
LIFECYCLE_VALUES = {"active", "archive", "archived", "legacy"}
...
def make_parser() -> argparse.ArgumentParser:
    ...
    for name, func, help_text in [
        ...
        ("lifecycle", cmd_lifecycle, "set a source note lifecycle relation"),
        ...
        if name == "lifecycle":
            sub.add_argument("id", help="10-digit note ID.")
            sub.add_argument("value", choices=sorted(LIFECYCLE_VALUES), help="Lifecycle relation to write into zk metadata.")
            sub.add_argument("--build", action="store_true", help="Build artifacts after updating lifecycle metadata.")
        if name == "context":
            sub.add_argument(
                "--all",
                action="store_true",
...
def cmd_lifecycle(args: argparse.Namespace) -> int:
    project = require_project(args)
    path = note_path_for_id(project, args.id)
    lifecycle = canonical_lifecycle(args.value)
    with file_lock(project.source.parent / ".source.lock"):
        text = path.read_text(encoding="utf-8")
        updated = set_note_lifecycle(text, lifecycle)
        if updated != text:
            atomic_write_text(path, updated)
    note = note_info_by_id(project, args.id)
    print(f"lifecycle: {note['id']}  {note['title']}  {lifecycle}")
    if getattr(args, "build", False):
        build_project(project)
        print(f"artifact:   {project.artifact}")
    else:
...
def cmd_check(args: argparse.Namespace) -> int:
    project = require_project(args)
    require_source_wiki(project)
    exit_status = run_zk_lsp(["--wiki-root", str(project.source), "check"], cwd=project.source, raise_on_failure=False)
    report = lifecycle_reference_report(project)
    if report:
        print(report, end="")
    return exit_status
...
def lifecycle_reference_report(project: Project) -> str:
    entries = lifecycle_reference_entries(project)
    if not entries:
        return ""
    parts: list[str] = []
    for entry in entries:
        label = "linked archived note" if entry.target_lifecycle == "archived" else "linked legacy note"
        parts.append(f"{entry.severity}: {label}\n")
        parts.append(f" ┌─ {entry.source}:{entry.line}:{entry.column}\n")
        parts.append(" │\n")
        parts.append(f"{entry.line} │ {entry.line_text}\n")
        suffix = ""
        if entry.relation_targets:
            suffix = " New ids: " + ", ".join(f"@{target}" for target in entry.relation_targets)
        parts.append(f" │ @{entry.target_id} is {entry.target_lifecycle}.{suffix}\n\n")
    return "".join(parts)


def lifecycle_reference_entries(project: Project) -> list[LifecycleReference]:
    note_paths = sorted((project.source / "note").glob("*.typ"))
    note_infos = [parse_note_info(path) for path in note_paths]
    notes_by_id = {str(info["id"]): info for info in note_infos}
    source_paths = [project.source / "index.typ", *note_paths]
    entries: list[LifecycleReference] = []
    for source_path in source_paths:
        if not source_path.exists():
            continue
        source_id = source_path.stem if source_path.parent.name == "note" else ""
        for ref in refs_in_source(source_path):
            target = notes_by_id.get(ref["id"])
            if target is None:
                continue
            lifecycle = normalized_lifecycle(str(target.get("lifecycle") or ""))
            if lifecycle not in {"archived", "legacy"}:
                continue
            relation_targets = tuple(str(item) for item in relation_targets_for_info(target))
            if source_id and source_id in relation_targets:
                continue
            if lifecycle == "legacy" and line_mentions_successor_after_ref(ref["line_text"], ref["end"], relation_targets):
                continue
            severity = "warning" if lifecycle == "archived" else "info"
            entries.append(
                LifecycleReference(
                    severity=severity,
                    source=source_path,
    ...
    return sorted(entries, key=lambda entry: (str(entry.source), entry.line, entry.column, entry.target_id))


def refs_in_source(path: Path) -> list[dict[str, object]]:
    refs: list[dict[str, object]] = []
    for line_number, raw_line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
        line = strip_typst_line_comment(raw_line)
        for match in re.finditer(r"@(\d{10})", line):
            refs.append(
                {
                    "id": match.group(1),
                    "line": line_number,
...
def normalized_lifecycle(value: str) -> str:
    return canonical_lifecycle(value)


def canonical_lifecycle(value: str) -> str:
    if value == "archive":
        return "archived"
    return value


def relation_targets_for_info(info: Mapping[str, object]) -> list[str]:
    metadata = info.get("metadata")
    if not isinstance(metadata, Mapping):
        return []
    value = metadata.get("relation-target")
    if isinstance(value, list):
        return [str(item) for item in value if str(item)]
    return []


def line_mentions_successor_after_ref(line: str, end: object, relation_targets: tuple[str, ...]) -> bool:
    if not relation_targets or not isinstance(end, int):
        return False
    after = strip_typst_line_comment(line[end:])
    return any(re.search(rf"@{re.escape(target)}(?!\d)", after) for target in relation_targets)
...
def list_note_summaries(project: Project) -> list[dict[str, object]]:
...
def note_info_by_id(project: Project, note_id: str) -> dict[str, object]:
    validate_note_id(note_id)
    ...
    metadata = raw_metadata if isinstance(raw_metadata, dict) and raw_metadata else fallback["metadata"]
    return {
        ...
        "metadata": metadata,
        "lifecycle": str(metadata.get("relation") or fallback["lifecycle"]),
    }
...
def parse_note_info(path: Path) -> dict[str, object]:
    text = path.read_text(encoding="utf-8")
    match = re.search(r"^=\s*(.*?)\s+<(\d{10})>\s*$", text, flags=re.MULTILINE)
    if not match:
        raise MemoryDreamError(f"invalid note header: {path}")
    metadata_text, _ = extract_metadata(text)
    metadata = parse_metadata(metadata_text, source=str(path))
    return {
        "title": match.group(1).strip(),
        "id": match.group(2),
        "path": str(path),
        "metadata": metadata,
        "lifecycle": str(metadata.get("relation") or ""),
    }


def parse_metadata(metadata: str, *, source: str) -> dict[str, object]:
    if not metadata.strip():
        return {}
    try:
        return dict(tomllib.loads(metadata))
    except tomllib.TOMLDecodeError as exc:
        raise MemoryDreamError(f"invalid zk metadata for {source}: {exc}") from exc


def set_note_lifecycle(text: str, lifecycle: str) -> str:
    lifecycle = canonical_lifecycle(lifecycle)
    if lifecycle not in LIFECYCLE_VALUES:
        raise MemoryDreamError(f"invalid lifecycle: {lifecycle}")
    located = locate_metadata_fence(text)
    if located is None:
        block = [
            "#let zk-metadata = toml(bytes(",
            "  ```toml",
            f'  relation = "{toml_escape(lifecycle)}"',
            "  ```",
            "))",
            "",
        ]
        suffix = "\n" if text else ""
        return "\n".join(block) + suffix + text
    lines, fence, end = located
    metadata_lines = lines[fence + 1 : end]
    updated_metadata = set_toml_relation(metadata_lines, lifecycle)
    parse_metadata("\n".join(updated_metadata), source="updated lifecycle metadata")
    updated_lines = lines[: fence + 1] + updated_metadata + lines[end:]
    trailing = "\n" if text.endswith("\n") else ""
    return "\n".join(updated_lines) + trailing


def locate_metadata_fence(text: str) -> tuple[list[str], int, int] | None:
    ...
    for idx, line in enumerate(lines):
        ...
        if fence is not None and stripped.startswith("```"):
            return lines, fence, idx
    return None


def set_toml_relation(lines: list[str], lifecycle: str) -> list[str]:
    updated: list[str] = []
    inserted = False
    relation_seen = False
    in_table = False
    for line in lines:
        ...
        if stripped.startswith("["):
            if not inserted:
                updated.append(f'  relation = "{toml_escape(lifecycle)}"')
                inserted = True
            in_table = True
            updated.append(line)
            continue
        if not in_table and top_level_toml_key(stripped) == "relation":
            if relation_seen:
                continue
            indent = line[: len(line) - len(line.lstrip())]
            updated.append(f'{indent}relation = "{toml_escape(lifecycle)}"')
            inserted = True
            relation_seen = True
            continue
        updated.append(line)
